...
Public Class hdImage : Implements IHttpHandler
    ...
    Public Sub ProcessRequest(ByVal context As HttpContext) Implements IHttpHandler.ProcessRequest
        ...
        If (IsNothing(context.Request.QueryString("PhotoUrl")) = False AndAlso
            ...
            Dim photo_url As String = ""

            photo_url = HttpContext.Current.Server.MapPath(PhotoUrl)
            ...
            Using wc As New WebClient()
                ...
                Catch ex As Exception
                    ...
                    Catch ex2 As Exception
                        Try
                            photo_url = "Upload_s/no_photo.gif"
                            stream = wc.OpenRead(photo_url)
        ...
        '---------
        Try
            Dim Width As Double = context.Request.QueryString("Width")
            Dim Height As Double = context.Request.QueryString("Height")
            Dim MyImage As System.Drawing.Image = System.Drawing.Image.FromStream(stream)

            MyImage = ReSize(MyImage, Width, Height, False)
    ...
    Public Function ReSize(ByVal image As Image, ByVal Width As Integer, ByVal Height As Integer, ByVal needToFill As Boolean) As System.Drawing.Image
        '#Region "много арифметики"
        Dim sourceWidth As Integer = image.Width
        Dim sourceHeight As Integer = image.Height
        Dim sourceX As Integer = 0
        ...
        Dim bmPhoto As System.Drawing.Bitmap = Nothing
        Try
            bmPhoto = New System.Drawing.Bitmap(Width + CInt(Math.Round(2 * destX)), destHeight + CInt(Math.Round(2 * destY)))
        Catch ex As Exception
            Throw New ApplicationException(String.Format("destWidth:{0}, destX:{1}, destHeight:{2}, desxtY:{3}, Width:{4}, Height:{5}", destWidth, destX, destHeight, destY, Width,
                Height), ex)
        End Try
        Using grPhoto As System.Drawing.Graphics = System.Drawing.Graphics.FromImage(bmPhoto)
            ...
            Dim x_start As Integer = (Width / 2) - (destWidth / 2)
            Dim y_start As Integer = 0
            Dim [to] As Rectangle = New System.Drawing.Rectangle(x_start, y_start, destWidth, destHeight)
            Dim from As Rectangle = New System.Drawing.Rectangle(sourceX, sourceY, sourceWidth, sourceHeight)
            'Console.WriteLine("From: " + from.ToString());
            'Console.WriteLine("To: " + to.ToString());
            grPhoto.DrawImage(image, [to], from, System.Drawing.GraphicsUnit.Pixel)
